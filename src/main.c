...
int writeTest() {
	...
	UINT bw;
	FIL fil;
	FRESULT res = f_open(&fil, "test.bin", FA_WRITE | FA_CREATE_ALWAYS);
	if (res != FR_OK) {
		printf("f_open test.bin failed: %d\n", res);
		return 0;
	}
	for (int i = 0; i < 1024; i++) {
		res = f_write(&fil, binbuf, 4096, &bw);
		if (res != FR_OK) {
...
int main(void) {
	...
	while (1) {
		...
		if (!strcmp(command, "h")) {
			...
			printf("w                  - write 'test.bin' with 4 MB zeros\n");
			continue;
		}
		...
		if (!strcmp(command, "w")) {
			...
			uint32_t elapsed = (msTicks - start) / 1000;
			uint32_t speed = 4096 * 1024 / elapsed / 1000;
			printf("time: %"PRIu32" s, speed: %"PRIu32" kB/s\n", elapsed,
					speed);
			continue;
		}
